...
XMLSensitizedProcessStatement::XMLSensitizedProcessStatement() {}
XMLSensitizedProcessStatement::~XMLSensitizedProcessStatement() {}

XMLDesignatorList *
XMLSensitizedProcessStatement::_get_sensitivity_list() {
  return dynamic_cast<XMLDesignatorList *>(get_sensitivity_list());
}

void
XMLSensitizedProcessStatement::xmlGeneration(XmlFile &file) {
  if (!bVisited) {
